...
function secondline_add_custom_types_in_archives( $query ) {
    if( is_author() && empty( $query->query_vars['suppress_filters'] ) ) {
        $query->set( 'post_type', array('post', 'podcast') );
        return $query;
    }
}
add_filter( 'pre_get_posts', 'secondline_add_custom_types_in_archives');
...
// Plugin updater 
$current_theme_directory = wp_get_theme();
$current_theme = $current_theme_directory->get( 'TextDomain' );
if( $current_theme == ('satchmo-secondline' || 'dixie-secondline' || 'gumbo-secondline' || 'bolden-secondline' || 'tusant-secondline')) {
	if( get_option($current_theme . '_license_key_status') == 'valid' ) {
		require SECONDLINE_THEME_ELEMENTS_PATH . 'inc/plugin-update-checker/plugin-update-checker.php';
		$myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
			'https://secondlinethemes.com/sltc/plugin.json',
			__FILE__, 'secondline-custom'
		);
	}
}